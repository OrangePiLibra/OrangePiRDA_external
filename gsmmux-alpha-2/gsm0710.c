...
static char *dev_symlink_prefix = NULL;
/*misc global vars */
...
char *create_symlink_name(int idx) {
    if (dev_symlink_prefix == NULL || idx < 1) {
        return NULL;
    }
    char* symLinkName  = malloc(strlen(dev_symlink_prefix)+255);
    sprintf(symLinkName, "%s%d", dev_symlink_prefix, idx - 1);
    return symLinkName;
}
...
static int serial_write(int fd, unsigned char *data, int length)
{
    int written = 0;
    int c = 0;
    ...
    while(written < length)
    {
        ...
        if (c < 0)
        {
            LOGMUX(LOG_ERR,"serial_write error, err = %d", c);
            return -1;
        }
        written += c;
    }
        ...
        unsigned char type)
{
    ...
    if (!cmux_mode)
    {
        ...
        c = serial_write(serial.fd, prefix, prefix_length);
        if (c != prefix_length)
        {
        ...
        if (length > 0)
        {
            ...
            c = serial_write(serial.fd, input, length);
            if (length != c)
            {
            ...
            //LOGMUX(LOG_ERR,"android->modem: channel = %d, length = %d/%d written", channel, c, length);
        }
        syslogdump(">s ", postfix,2); /* syslogdump for basic mode */
        c = serial_write(serial.fd, postfix, 2);
        if (c != 2)
        {
    ...
    else/* cmux_mode */
    {
        ...
        c = serial_write(serial.fd, serial.adv_frame_buf, offs);
        if (c != offs)
        {
...
int restart_pty_interface(Channel* channel)
{
    if (channel->fd < 0) // is this channel free?
    {
        ...
        channel->ptslinkname = strdup(create_symlink_name(channel->id));
        LOGMUX(LOG_INFO, "id = %d,pstname  = %s, linkname = %s\n",channel->id,channel->ptsname, channel->ptslinkname);
        unlink(channel->ptslinkname);
        if (symlink(channel->ptsname, channel->ptslinkname) != 0) SYSCHECK(-1);
    ...
    if (serial.state == MUX_STATE_MUXING)
        for (i=1;i<GSM0710_MAX_CHANNELS;i++)
            if (channellist[i].fd < 0) // is this channel free?
            {
                ...
                channellist[i].ptslinkname = strdup(create_symlink_name(i));
                LOGMUX(LOG_INFO, "i = %d,pstname  = %s, linkname = %s\n",i,channellist[i].ptsname, channellist[i].ptslinkname);
                unlink(channellist[i].ptslinkname);
                if (symlink(channellist[i].ptsname, channellist[i].ptslinkname) != 0) SYSCHECK(-1);
        ...
        GSM0710_Buffer * buf)
{
    ...
    if (local_datacount >= length_needed) /* enough data stored for 0710 frame header+footer? */
    {
        ...
        {
            memset(frame, 0x00, sizeof(GSM0710_Frame));
        ...
        int to)
{
    ...
    SYSCHECK(wrote = serial_write(serial_device_fd, cmd, strlen(cmd)));
        ...
        if ((GSM0710_FRAME_IS(GSM0710_TYPE_UI, frame) || GSM0710_FRAME_IS(GSM0710_TYPE_UIH, frame)))
        {
            ...
            if (frame->channel > 0)
            {
                ...
                if ((write_result = write(channellist[frame->channel].fd, frame->data, frame->length)) >= 0)
                {
                    ...
                    //LOGMUX(LOG_ERR, "modem->android %d/%d bytes written to %s",write_result,frame->length,channellist[frame->channel].ptslinkname);
            ...
            case MUX_STATE_MUXING:
                {
                    ...
                    {
                        do {
                            len = read(serial->fd, buf, min(length, sizeof(buf)));
                        } while (len < 0 && ((errno == EAGAIN)||(errno == EINTR)));
        ...
        )
{
    ...
    SYSCHECK(serial->fd = open(serial->devicename, O_RDWR | O_NOCTTY | O_NDELAY));
...
int main(int argc,char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "dov:s:t:l:p:f:n:h?m:b:P:")) > 0)
    {
        switch (opt)
        {
            case 'v':
